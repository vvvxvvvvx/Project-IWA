...
function renderDonut(topStations) {
    ...
    const labels = topStations.map((s) => s.name || s.stn); const values = topStations.map((s) => Number(s.reading_count));
...
function updateMetrics(overview) {
    setText('metricStationCount', overview.station_count); setText('metricReadingCount', overview.reading_count); setText('metricAverageTemp', overview.average_temp); setText('metricPeakCount', overview.peak_count); setText('metricMissingCount', overview.missing_count);
...
function rowsToHtml(rows, type) {
    if (type === 'latest') return rows.map((row) => `<tr><td>${row.name}</td><td>${formatValue(row.location_label)}</td><td>${formatValue((row.measured_at || '').replace('T',' ').replace('+00:00',' UTC'))}</td><td>${formatValue(row.temp)}</td><td>${formatValue(row.dewp)}</td><td>${formatValue(row.wdsp)}</td><td>${formatValue(row.visib)}</td><td>${formatValue(row.prcp)}</td><td>${formatStatusBadge(row)}</td></tr>`).join('');
    if (type === 'stations') return rows.map((row) => `<tr><td>${row.stn || row.name}</td><td>${row.name}</td><td>${formatValue(row.location_label)}</td><td>${row.lat != null ? `${row.lat}, ${row.lon}` : '-' }</td><td>${formatValue((row.measured_at || '').replace('T',' ').replace('+00:00',' UTC'))}</td><td>${formatValue(row.temp)}</td><td>${formatValue(row.avg_temp)}</td><td>${formatValue(row.reading_count)}</td><td>${formatStatusBadge(row)}</td></tr>`).join('');
    ...
    if (type === 'flagged') return rows.map((row)=>`<tr><td>${row.name}</td><td>${formatValue((row.measured_at || '').replace('T',' ').replace('+00:00',' UTC'))}</td><td>${formatValue(row.temp)}</td><td>${Number(row.has_missing_data||0)===1?'<span class="status-badge warning">Missing data</span>':'<span class="status-badge info">Temp peak</span>'}</td></tr>`).join('');
    return '';
}
function updateSection(id, html) { const el = document.getElementById(id); if (el) el.innerHTML = html; }
function updateTopList(rows) { const list = document.getElementById('topStationsList'); if (list) list.innerHTML = rows.map((s)=>`<li><span class="source-name">${s.name || s.stn}</span><span class="source-value">${s.reading_count}</span></li>`).join(''); }
function renderDashboard(data) {
    ...
    updateSection('stationsTableBody', rowsToHtml(data.stations || [], 'stations'));
