...
let dashboardStationsState = Array.isArray(bootstrap.stations) ? [...bootstrap.stations] : [];

function formatValue(value) {
    return value === null || value === undefined || value === '' ? '-' : String(value);
}

function stationStatusKey(row) {
    if (Number(row.has_missing_data || 0) === 1) return 'missing';
    if (Number(row.is_temp_peak || 0) === 1) return 'peak';
    return 'ok';
}

function stationStatusLabel(row) {
    if (stationStatusKey(row) === 'missing') return 'Missing data';
    if (stationStatusKey(row) === 'peak') return 'Peak';
    return 'OK';
}
...
function renderDonut(topStations) {
    const element = document.getElementById('stationChart');
    if (!element) return;
    const labels = topStations.map((s) => s.stn);
    const values = topStations.map((s) => Number(s.reading_count));
    if (stationChart) {
...
function updateMetrics(overview) {
    setText('metricStationCount', overview.station_count);
    setText('metricReadingCount', overview.reading_count);
    setText('metricAverageTemp', overview.average_temp);
    setText('metricMinTemp', overview.min_temp);
    setText('metricMaxTemp', overview.max_temp);
    setText('metricPeakCount', overview.peak_count);
    setText('metricMissingCount', overview.missing_count);
    const stationCount = Math.max(Number(overview.station_count || 0), 1);
    const readingCount = Math.max(Number(overview.reading_count || 0), 1);
    const missing = Number(overview.missing_count || 0);
    const peaks = Number(overview.peak_count || 0);
    const health = Math.max(0, 100 - Math.min(100, missing * 2));
    const coverage = Math.min(100, Math.round((readingCount / Math.max(1, stationCount * 10)) * 100));
    const stability = Math.max(0, 100 - Math.min(100, peaks * 5));
    [['kpiHealthValue','kpiHealthBar',health],['kpiCoverageValue','kpiCoverageBar',coverage],['kpiStabilityValue','kpiStabilityBar',stability]].forEach(([valueId, barId, value]) => {
...
function rowsToHtml(rows, type) {
    ...
    if (type === 'stations') return rows.map((row) => `<tr><td><a href="/stations/${row.stn}">${row.stn}</a></td><td>${formatValue(row.name)}</td><td>${formatValue(row.location_label)}</td><td>${formatValue((row.measured_at || '').replace('T',' ').replace('+00:00',' UTC'))}</td><td>${formatValue(row.temp)}</td><td>${formatValue(row.reading_count)}</td><td>${formatStatusBadge(row)}</td></tr>`).join('');
...
function updateTopList(rows) {
    const list = document.getElementById('topStationsList');
    if (list) list.innerHTML = rows.map((s)=>`<li><span class="source-name">${s.name}</span><span class="source-value">${s.reading_count}</span></li>`).join('');
}

function applyDashboardStationFilters() {
    const body = document.getElementById('stationsTableBody');
    if (!body) return;
    const query = (document.getElementById('dashboardStationSearch')?.value || '').trim().toLowerCase();
    const status = document.getElementById('dashboardStationStatus')?.value || 'all';
    const filtered = dashboardStationsState.filter((row) => {
        if (status !== 'all' && stationStatusKey(row) !== status) {
            return false;
        }
        if (query === '') {
            return true;
        }
        const haystack = [row.stn, row.name, row.location_label].map((value) => String(value || '').toLowerCase()).join(' ');
        return haystack.includes(query);
    });
    updateSection('stationsTableBody', rowsToHtml(filtered, 'stations'));
    setText('dashboardStationVisibleCount', filtered.length);
}

function setupDashboardStationFilters() {
    const form = document.getElementById('dashboardStationFilterForm');
    if (!form) return;
    form.addEventListener('submit', (event) => {
        event.preventDefault();
        applyDashboardStationFilters();
    });
    document.getElementById('dashboardStationSearch')?.addEventListener('input', applyDashboardStationFilters);
    document.getElementById('dashboardStationStatus')?.addEventListener('change', applyDashboardStationFilters);
    document.getElementById('dashboardStationReset')?.addEventListener('click', () => {
        const search = document.getElementById('dashboardStationSearch');
        const status = document.getElementById('dashboardStationStatus');
...
function renderDashboard(data) {
    dashboardStationsState = Array.isArray(data.stations) ? [...data.stations] : [];
    updateMetrics(data.overview || {});
    renderLine(data.chart_points || []);
    renderDonut(data.top_stations || []);
    updateTopList(data.top_stations || []);
    updateSection('latestReadingsBody', rowsToHtml(data.latest_readings || [], 'latest'));
    updateSection('batchTableBody', rowsToHtml(data.latest_batches || [], 'batches'));
    updateSection('flaggedReadingsBody', rowsToHtml(data.flagged_readings || [], 'flagged'));
    updateSection('correctionsBody', rowsToHtml(data.recent_corrections || [], 'corrections'));
    updateSection('originalsBody', rowsToHtml(data.recent_originals || [], 'originals'));
    applyDashboardStationFilters();
}
...
async function refreshDashboard() {
    try {
        ...
        if(indicator){indicator.textContent='Live'; indicator.classList.remove('warning-state');}
    } catch (error) {
        const indicator = document.getElementById('liveIndicator');
        if(indicator){indicator.textContent='Verbinding controleren'; indicator.classList.add('warning-state');}
        console.error(error);
    }
}

setupTabs();
setupDashboardStationFilters();
renderDashboard(bootstrap);
